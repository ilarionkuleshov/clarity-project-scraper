...
class FinancesRepliesConsumer(Consumer):
    def __init__(self):
        super().__init__()
        self.queue_name = self.project_settings.get("FINANCES_REPLIES")

    def build_message_store_stmt(self, message_body):
        values = {k:v for k,v in message_body.items() if k not in ["id", "edr"]}
